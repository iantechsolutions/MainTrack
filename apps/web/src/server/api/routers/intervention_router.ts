...
export const interventionsRouter = createTRPCRouter({
  ...
  setStatus: protectedProcedure.input(intervSetStatusSchema).mutation(async ({ ctx, input }) => {
    const selfId = ctx.session.user.id;
    const interv = await db.query.interventions.findFirst({
      where: eq(schema.interventions.Id, input.intId),
    });
  ...
  edit: protectedProcedure.input(intervEditSchema).mutation(async ({ ctx, input }) => {
    const selfId = ctx.session.user.id;
    const interv = await db.query.interventions.findFirst({
      where: eq(schema.interventions.Id, input.intId),
    });
    ...
    if (typeof input.asignadoUserId === "string") {
      asignado =
        (await db.query.usuariosOrganizaciones.findFirst({
          where: and(
