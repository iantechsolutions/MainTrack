...
export const orgRouter = createTRPCRouter({
  create: protectedProcedure.input(schemaOrgPut).mutation(async ({ input, ctx }) => {
    ...
    const userOrg = await db
      .insert(schema.usuariosOrganizaciones)
      .values({
        orgId: org.Id,
        userId: selfUser.Id,
    ...
    if (userOrg.length < 1) {
      await db.delete(schema.organizaciones).where(eq(schema.organizaciones.Id, org.Id));
      throw new TRPCError({ code: "INTERNAL_SERVER_ERROR" });
    }
  ...
  edit: protectedProcedure.input(schemaOrgPatch).mutation(async ({ input, ctx }) => {
    const selfId = ctx.session.user.id;
    const { orgId } = input;
    ...
    const orgUserEntry = await db.query.usuariosOrganizaciones.findFirst({
      where: and(eq(schema.usuariosOrganizaciones.orgId, org.Id), eq(schema.usuariosOrganizaciones.userId, selfId)),
    });
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const orgUserEntries = await db.query.usuariosOrganizaciones.findMany({
        ...
        where: and(eq(schema.usuariosOrganizaciones.orgId, org.Id)),
      });

      // saca a todos los usuarios
      await db.delete(schema.usuariosOrganizaciones).where(eq(schema.usuariosOrganizaciones.orgId, org.Id));
      ...
      await db.transaction(async (tx) => {
        await tx.delete(schema.usuariosOrganizaciones).where(eq(schema.usuariosOrganizaciones.orgId, org.Id));
        await tx.delete(schema.equipmentCategories).where(eq(schema.equipmentCategories.orgId, org.Id));
        await tx.delete(schema.equipmentPhotos).where(eq(schema.equipmentPhotos.orgId, org.Id));
        await tx.delete(schema.equipment).where(eq(schema.equipment.orgId, org.Id));
        await tx.delete(schema.documentTypes).where(eq(schema.documentTypes.orgId, org.Id));
        await tx.delete(schema.documents).where(eq(schema.documents.orgId, org.Id));
        await tx.delete(schema.ots).where(eq(schema.ots.orgId, org.Id));
        await tx.delete(schema.interventions).where(eq(schema.interventions.orgId, org.Id));
        await tx.delete(schema.organizaciones).where(eq(schema.organizaciones.Id, org.Id));
      });
  ...
  get: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const orgUserEntry = await db.query.usuariosOrganizaciones.findFirst({
        where: and(eq(schema.usuariosOrganizaciones.orgId, orgId), eq(schema.usuariosOrganizaciones.userId, selfId)),
      });
  ...
  listUsers: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      const orgUsers = await db.query.usuariosOrganizaciones.findMany({
        where: and(eq(schema.usuariosOrganizaciones.orgId, input.orgId)),
      });
      ...
      return usersDetailed.map((entry) =>
        entry.profile
          ? {
  ...
  inviteUser: protectedProcedure.input(schemaOrgInvite).mutation(async ({ input, ctx }) => {
    const selfId = ctx.session.user.id;
    ...
    const orgUserEntry = await db.query.usuariosOrganizaciones.findFirst({
      ...
      where: and(eq(schema.usuariosOrganizaciones.orgId, input.orgId), eq(schema.usuariosOrganizaciones.userId, userId)),
    });
  ...
  join: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      if (
        typeof tokenClaims.orgId !== "string" ||
        typeof tokenClaims.targetUserId !== "string" ||
        typeof tokenClaims.fromUserId !== "string"
      ) {
        throw new TRPCError({ code: "BAD_REQUEST" });
      }
  ...
  removeUser: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db
        .delete(schema.usuariosOrganizaciones)
        .where(and(eq(schema.usuariosOrganizaciones.orgId, org.Id), eq(schema.usuariosOrganizaciones.userId, otherUser.Id)));
  ...
  setRole: protectedProcedure.input(schemaOrgSetRole).mutation(async ({ input, ctx }) => {
    const selfId = ctx.session.user.id;
    const selfUser = await db.query.users.findFirst({
      where: eq(schema.users.Id, selfId),
    });
    ...
    return await db
      .update(schema.usuariosOrganizaciones)
      .set({
  ...
  select: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      const orgUserEntry = await db.query.usuariosOrganizaciones.findFirst({
        where: and(eq(schema.usuariosOrganizaciones.orgId, org.Id), eq(schema.usuariosOrganizaciones.userId, selfUser.Id)),
      });
