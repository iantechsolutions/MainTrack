...
export const eqTypeRouter = createTRPCRouter({
  create: protectedProcedure.input(eqTypeCreateSchema).mutation(async ({ ctx, input }) => {
  ...
  delete: protectedProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      await db.delete(schema.equipmentCategories).where(eq(schema.equipmentCategories.Id, eqType.Id));
  ...
  listFiltered: protectedProcedure.input(eqTypeListSchema).mutation(async ({ input, ctx }) => {
    const selfId = ctx.session.user.id;
    const userOrgEntry = await userInOrg(selfId, input.orgId);
    if (!userOrgEntry) {
      throw new TRPCError({ code: "NOT_FOUND" });
