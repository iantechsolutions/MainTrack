...
export const userSelectedOrg = async (
  id: string,
): Promise<
  | [
...
> => {
  ...
  const orgSelfEntry = await db.query.usuariosOrganizaciones.findFirst({
    ...
    where: and(eq(schema.usuariosOrganizaciones.orgId, selfUser.orgSel), eq(schema.usuariosOrganizaciones.userId, selfUser.Id)),
  });

  if (orgSelfEntry === undefined) {
    return null;
  } else {
    return [selfUser, orgSelfEntry.organizacion, orgSelfEntry];
  }
};
...
export const userInOrg = async (userId: string, orgId: string): Promise<InferSelectModel<typeof schema.usuariosOrganizaciones> | null> => {
  const orgSelfEntry = await db.query.usuariosOrganizaciones.findFirst({
    where: and(eq(schema.usuariosOrganizaciones.orgId, orgId), eq(schema.usuariosOrganizaciones.userId, userId)),
  });
